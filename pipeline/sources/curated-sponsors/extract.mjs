/**
 * Hand-verified sponsor records.
 *
 * No network access. Reads JSON from data/curated/sponsors/ so a person can
...
const DIR = join(process.cwd(), 'data', 'curated', 'sponsors');
...
export default async function extract({ log }) {
  ...
  log(`${records.length} curated sponsor(s) from ${files.length} file(s)`);
  return records;
}
